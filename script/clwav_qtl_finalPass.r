...
fq.auto <- fitqtl(multisim, qtl=qtl.auto, formula=f1, model='binary')
...
# refine positions
rqtl <- refineqtl(multisim, qtl=qtl.auto, formula=f1)
...
# plots
pdf("/Users/Wen-Juan/Dropbox (Amherst College)/Amherst_postdoc/github/Decaytrait_qtl/output/qtl_lod_em_lodprofile.pdf",  width=12, height=6)
par(mfrow=c(1,2)) 
plot(rqtl)
plotLodProfile(rqtl)
dev.off()
...
pdf("/Users/Wen-Juan/Dropbox (Amherst College)/Amherst_postdoc/github/Decaytrait_qtl/output/qtl_lod_em_effectplot.pdf",  width=12, height=6)
par(mfrow=c(1,2)) 
par(mar=c(5,5,4,3))
...
capture.output(summary(final.fit, alpha=0.05, pvalues=TRUE, format="allpheno"), file=file.path(outpath, paste("pheno",'em_PVE_table.txt', sep="_")))
